...
class PlaywrightController extends EventEmitter {
  constructor(config, logger) {
    ...
    // Cooldown timers — reset edge state after delay so consecutive same-events are detected
    this.cooldowns = {};
  }
  ...
  stopMonitoring() {
    ...
    for (const key of Object.keys(this.cooldowns)) {
      if (this.cooldowns[key]) clearTimeout(this.cooldowns[key]);
    }
    this.cooldowns = {};
  }
  ...
  async pollDOM() {
    ...
    // Cooldown resets lastState after delay so consecutive same-events are detected
    if (state.bust && !this.lastState.bust) {
      this.logger.info('Playwright: BUST detekterat!');
      this.emit('bust');
      this.startCooldown('bust', 3000);
    }

    if (state.legWon && !this.lastState.legWon) {
      this.logger.info('Playwright: LEG WON detekterat!');
      this.emit('leg-won');
      this.startCooldown('legWon', 3000);
    }

    if (state.setWon && !this.lastState.setWon) {
      this.logger.info('Playwright: SET WON detekterat!');
      this.emit('set-won');
      this.startCooldown('setWon', 3000);
    }

    this.lastState = state;
  }

  startCooldown(key, delayMs) {
    if (this.cooldowns[key]) clearTimeout(this.cooldowns[key]);
    this.cooldowns[key] = setTimeout(() => {
      this.lastState[key] = false;
      this.cooldowns[key] = null;
      this.logger.debug(`Playwright: Cooldown reset för ${key}`);
    }, delayMs);
  }
