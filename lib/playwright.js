...
class PlaywrightController extends EventEmitter {
  ...
  async muteAudio() {
    if (!this.page) return;
    try {
      await this.page.evaluate(() => { window.__scoliaMuted = true; });
  ...
  async unmuteAudio() {
    if (!this.page) return;
    try {
      await this.page.evaluate(() => { window.__scoliaMuted = false; });
    } catch { /* page might be closed */ }
  }
