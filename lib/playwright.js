...
const { nextBackoffDelay } = require('./backoff');
...
class PlaywrightController extends EventEmitter {
  constructor(config, logger, notifier = null) {
    ...
    this.running = false;
    this.restartAttempts = 0; // Backoff-räknare för restart, nollställs vid lyckad start
    ...
    // Join-screen auto-reload — Scolia auto-enters the board on a fresh page load,
    // but if we loaded /game while the board was offline we get stuck on the
    // join-board screen forever. Reload periodically to self-heal once it's online.
    this.joinScreenSince = 0;
    this.lastJoinReloadAt = 0;
  ...
  // Reload the page and reset edge-detection state. A reload usually means a new
  // game (bustCount back to 0, winner tiles gone), so stale lastState counts must
  // be cleared or the increase-based comparison would miss the next bust/win.
  async reloadPage() {
    ...
    try {
      ...
      this.logger.warn(`Playwright: Reload fel: ${err.message}`);
    }
  }
  ...
  async pollDOM() {
    ...
    const state = await this.page.evaluate(() => {
      ...
      // Join-board screen — shown when not connected to a board (e.g. board was
      // offline at page load). A fresh reload auto-enters the board once online.
      const onJoinScreen = !!document.querySelector('[class*="joinBoardForm"]');

      return { bustCount, legWon, setWon, backToSetup, onJoinScreen };
    });
    ...
    // Handle join-board screen — reload to auto-enter the board once it's online.
    // Debounced so we reload at most once per joinReloadMs while stuck here.
    if (state.onJoinScreen) {
      const now = Date.now();
      if (!this.joinScreenSince) {
        this.joinScreenSince = now;
        this.logger.info('Playwright: På join-skärmen (ej ansluten till tavla) — väntar på att tavlan blir online');
      }
      const reloadAfterMs = this.config.joinReloadMs || 30000;
      if (now - this.joinScreenSince > reloadAfterMs && now - this.lastJoinReloadAt > reloadAfterMs) {
        this.lastJoinReloadAt = now;
        this.logger.info('Playwright: Laddar om för att auto-ansluta till tavlan');
        await this.reloadPage();
      }
      return;
    }
  ...
  async restart() {
    ...
    if (ok) {
      this.restartAttempts = 0; // Lyckad start — nollställ backoff
      if (this.notifier) this.notifier.recover('playwright-down', 'Playwright/browser körs igen');
      this.startMonitoring();
    } else {
      // Exponentiell backoff istället för fast 10s, så en permanent kraschad
      // Chromium inte öppnar en ny process var 10:e sekund i all evighet.
      this.restartAttempts += 1;
      const delay = nextBackoffDelay(this.restartAttempts, {
        baseMs: this.config.restartBaseDelayMs || 10000,
        maxMs: this.config.restartMaxDelayMs || 300000,
        jitterMs: 2000,
      });
      const secs = (delay / 1000).toFixed(0);
      const alertAfter = this.config.restartAlertAfter || 3;
      if (this.restartAttempts >= alertAfter) {
        this.logger.error(`Playwright: Kunde inte starta om (försök #${this.restartAttempts}) — kontrollera att Chromium/skärm fungerar. Nytt försök om ${secs}s`);
        if (this.notifier) this.notifier.alert('playwright-down', `Playwright kunde inte starta om (försök #${this.restartAttempts}). Bust/win-ljud funkar inte.`);
      } else {
        this.logger.error(`Playwright: Kunde inte starta om — försöker igen om ${secs}s (försök #${this.restartAttempts})`);
      }
      setTimeout(async () => {
        try {
          await this.restart();
