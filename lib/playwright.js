...
class PlaywrightController extends EventEmitter {
  ...
  stopMonitoring() {
    ...
    if (this.finishBtnTimer) {
      clearTimeout(this.finishBtnTimer);
    ...
    if (this.dumpStopTimer) {
      clearTimeout(this.dumpStopTimer);
      this.dumpStopTimer = null;
    }
  }

  async dumpPostGameDOM(label) {
    if (!this.page) return;
    try {
      const dump = await this.page.evaluate(() => {
        const truncate = (s, n) => (s || '').toString().trim().slice(0, n);
        const clsOf = el =>
          typeof el.className === 'string' ? truncate(el.className, 120) : null;

        const buttons = Array.from(document.querySelectorAll('button'))
          .map(b => ({
            text: truncate(b.innerText, 60),
            id: b.id || null,
            cls: clsOf(b),
          }))
        ...
        const matches = Array.from(
          document.querySelectorAll(
            '[id*="post-game" i],[id*="back" i],[id*="setup" i],[id*="finish" i],[id*="stats" i],[class*="postGame" i],[class*="backToSetup" i],[class*="statsPage" i],[class*="finish" i]'
          )
        ).map(el => ({
      ...
      this.logger.info(`[DOM-DUMP ${label}] URL: ${dump.url}`);
      this.logger.info(`[DOM-DUMP ${label}] Buttons (${dump.buttons.length}):`);
      dump.buttons.forEach(b =>
        this.logger.info(`  btn "${b.text}" id=${b.id} cls=${b.cls}`)
      );
      this.logger.info(`[DOM-DUMP ${label}] Post-game träffar (${dump.matches.length}):`);
      dump.matches.forEach(el =>
        this.logger.info(`  <${el.tag}> id=${el.id} cls=${el.cls} text="${el.text}"`)
      );
    } catch (err) {
      this.logger.warn(`[DOM-DUMP ${label}] fel: ${err.message}`);
    }
  }

  startPostGameDump(label) {
    if (this.dumpInterval) clearInterval(this.dumpInterval);
    if (this.dumpStopTimer) clearTimeout(this.dumpStopTimer);

    this.dumpPostGameDOM(`${label} T+0s`);
    let elapsed = 0;
    this.dumpInterval = setInterval(() => {
      elapsed += 5;
      this.dumpPostGameDOM(`${label} T+${elapsed}s`);
    }, 5000);
    this.dumpStopTimer = setTimeout(() => {
      if (this.dumpInterval) {
        clearInterval(this.dumpInterval);
        this.dumpInterval = null;
      }
      this.logger.info(`[DOM-DUMP ${label}] klar (stoppat efter 90s)`);
    }, 90000);
  }
  ...
  async pollDOM() {
    ...
    const state = await this.page.evaluate(() => {
      ...
      return { bustCount, legWon, setWon, finishBtn, backToSetup, needsBoardSelect };
    });
    ...
    // Första reloaden landar ibland i ett halvinitierat state där DOM:en visar
    // nya gamet men Scolias realtime-WS aldrig etablerades. En andra reload
    // efter några sekunder (när SPA:n är helt i nya gamet) återupplivar den.
    if (state.backToSetup) {
      ...
      this.logger.info('Playwright: Andra reload för att återuppliva realtime');
      await this.page.reload({ waitUntil: 'networkidle', timeout: 30000 });
      return;
    }
